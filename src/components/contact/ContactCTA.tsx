...
import { QUICK_LINKS } from "@/lib/contact/constants";
          ...
          <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:justify-center sm:gap-4">
            {QUICK_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="flex items-center justify-center gap-2 rounded-full border border-border bg-glass px-5 py-2.5 font-body text-sm text-text-muted transition-colors duration-200 hover:border-gold hover:text-gold-light"
              >
      ...
      <section className="relative overflow-hidden border-t border-border bg-dark-3 py-20 sm:py-24">
        ...
        >
          ...
          <p className="mt-3 font-body text-base text-text-muted">
            Maharashtra ki best cab service — ek call door hai!
          </p>
